...
SITE = kalamarsite.create_site(PROJECTS_PATH)
CONFIG = {}
...
projects = (
    project for project in os.listdir(PROJECTS_PATH)
...
def before_request():
    ...
    g.variables.update(
        {'project_name': g.project_name, 'helpers': helpers})
...
def csstyle_stylesheet():
    ...
    filenames = (
        ...
        os.path.join(PROJECTS_PATH, g.project_name, local_filename))
...
def css_static_files(path):
    """CSS static files."""
    filenames = (
        os.path.join(PROJECTS_PATH, g.project_name, 'static', 'css', path),
